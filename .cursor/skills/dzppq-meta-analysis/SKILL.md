...
- Card analysis has a hard start date of 2026-07-27. From `screenshots.0727` onward, every shared-icon card is identified by detail OCR and must remain a concrete independent ranking key. Never merge concrete names (including SSS, QUALITY, FAST/XXB, 开攒/大亨, 波纹/利己, 礼包, 装备共鸣, 巨神兵/迅迅迅捷双剑, or 死亡摇滚/摇盒高手), and never overwrite them using final-board hero/equipment inference. Unresolved detail captures remain `unknown`. `蓝·半步满级` / `蓝·满级玩家` were not detail-captured in 0727, so their 0727 shared-template slots are `unknown` and their first valid batch is 0728. Shared-template maintenance steps are documented in `README.md` under “共享卡牌模板开发”.

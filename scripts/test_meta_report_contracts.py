...
class MetaReportContractTests(unittest.TestCase):
    ...
    def test_yellow_fortune_template_expands_to_two_logical_catalog_keys(self) -> None:
        self.assertEqual(
            MODULE.MERGED_TEMPLATE_EXPANSIONS["黄·未卜先知pro"],
            ["黄·城墙", "黄·未卜先知pro"],
        )

        yellow_catalog = MODULE.load_report_card_catalog()["黄"]
        self.assertIn("黄·城墙", yellow_catalog)
        self.assertIn("黄·未卜先知pro", yellow_catalog)
